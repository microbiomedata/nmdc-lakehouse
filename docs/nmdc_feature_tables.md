...
**Status: prototype.** These commands plan, sample, check and convert NMDC annotation feature
files into local Parquet shaped like the draft BER feature model
(https://github.com/turbomam/feature-table-corpus/blob/main/model/schema/ber_feature_model.yaml).
Nothing here uploads to BERDL (the KBase lakehouse). A bulk run over every annotation run has not
been done.
...
## What is loaded, and what is skipped

Measured on a 50-run sample on 2026-09-23 (see [Sample results](#sample-results)).

| `data_object_type` | treatment | reason |
|---|---|---|
| Functional Annotation GFF | loaded as genome features, `coordinate_system` `contig` | the selected gene and RNA calls with every functional key |
| Pfam, COG, TIGRFam, SMART, CATH FunFams, SUPERFam, KO_EC Annotation GFF | loaded as hits, `coordinate_system` `protein`, `parent` = the gene | only source of hit coordinates, scores, e-values and repeated hits |
| Structural Annotation GFF | skipped | the Functional Annotation GFF minus its functional keys, row for row |
| Annotation KEGG Orthology, Annotation Enzyme Commission | skipped | the same (gene, accession) pairs as KO_EC Annotation GFF |
| Product Names | skipped | `product` and `product_source` of the Functional Annotation GFF; see the RNA exception below |
| Prodigal, GeneMark, tRNA, RFAM, CRT Annotation GFF | skipped by default | selected calls repeat the Functional Annotation GFF; `--include-unselected` adds the rest, see below |
| Contig Mapping File, Scaffold Lineage tsv | loaded into `contigs.parquet` | assembly contig ID and per-contig lineage |
| Gene Phylogeny tsv, Crispr Terms | not loaded | not decided |

Accession keys in the Functional Annotation GFF (`pfam`, `cog`, `ko`, `ec_number` and the rest)
are dropped from a run's `attributes` only when that run's hit file exists and matched them
exactly. Otherwise they stay.
...
- **Old pipeline versions change coordinates after selection.** In v1.0.2 and v1.0.4 runs, some
  ...
  as 1 to 177). `--include-unselected` is refused for such runs, because it would label the source
  of a selected feature as unselected.
- **Old pipeline versions repeat IDs.** An RFAM hit over one interval on both strands gets one
  `ID` twice. The converter appends the strand to those feature IDs and keeps the original `ID`
  in `attributes`.
...
`source_data_object_type`, which the model does not have.
...
<!-- verified: 2026-09-23 planned 4,607 runs, sampled 50 runs (1,024 files, 42.55 GiB), downloaded in 9.1 minutes, checked and converted them locally -->
```bash
just feature-plan
...
Conversion of the 50 runs took 24 minutes on one core. It read 23.32 GiB of source files and wrote
3.38 GiB of Parquet: 85,834,048 features, of which 23,107,955 are genome features and 62,726,093
are hits. No feature ID repeats within a run, 32 were renamed for the strand, and every hit's
`parent` is a genome feature in the same run.

## Not done

- Upload to BERDL.
- A run over all 4,607 selected runs. About 5,028 GB of source files would be read for the loaded
  types (Functional Annotation GFF and the seven hit GFFs). The commands here convert from a full
  download cache; a streaming mode that downloads, converts and deletes one run at a time does not
  exist yet.
- Contig lengths, and the Gene Phylogeny and Crispr Terms files.
