# biosample_to_annotation_run — precomputed provenance table

## Purpose

`nmdc_metadata.biosample_to_annotation_run` maps every NMDC biosample to the
annotation workflow runs that produced results from it (or from derived
material). It is the join bridge between the graph metadata (`nmdc_metadata.*`)
and the per-gene/per-compound annotation results (`nmdc_results.annotation_kegg_orthology`,
`nmdc_results.annotation_enzyme_commission`, etc.).

**The table does not store individual annotation hits.** It stores one row per
(biosample, workflow run) pair. Annotation-level queries join this table to the
live `nmdc_results` tables on `workflow_run_id`.
...
JOIN   nmdc_results.annotation_kegg_orthology ko
         ON ko.workflow_run_id = b2ar.workflow_run_id
WHERE  b2ar.biosample_id = 'nmdc:bsm-11-xyz'
```

### KO hit counts per biosample, filtered to direct sequencing (no processing)

```sql
SELECT b2ar.biosample_id, COUNT(*) AS n_hits
FROM   nmdc_metadata.biosample_to_annotation_run b2ar
JOIN   nmdc_results.annotation_kegg_orthology ko
         ON ko.workflow_run_id = b2ar.workflow_run_id
WHERE  ko.annotation_id = 'KO:K00001'
  AND  b2ar.n_hops = 2   -- direct: DataGeneration.has_input → Biosample
GROUP  BY 1
ORDER  BY 2 DESC
```
...
```sql
SELECT b2ar.has_pooling, COUNT(DISTINCT b2ar.biosample_id) AS n_biosamples,
       COUNT(*) AS n_ko_hits
FROM   nmdc_metadata.biosample_to_annotation_run b2ar
JOIN   nmdc_results.annotation_kegg_orthology ko
         ON ko.workflow_run_id = b2ar.workflow_run_id
...
Run it once after each NMDC data load. The notebook uses an iterative BFS walk
over the precomputed `nmdc_metadata.graph_edges` table — one flat JOIN per hop
level rather than `WITH RECURSIVE` (which exceeds Trino's 150-stage planner
limit at this scale). `graph_edges` is built and dropped within the same
notebook run.
...
**Detection:** the preflight cell in the notebook runs this automatically:
...
| `biosample_to_annotation_run` | annotation workflows only | yes | yes |

Use `biosample_to_annotation_run` when you need to join to `nmdc_results` or
filter by processing chain shape. Use `workflow_to_biosample` (if built) for
broader coverage across all workflow types without the flag overhead.
