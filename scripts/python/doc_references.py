...
#: Ends a declaration early, so a file can exempt two paragraphs without exempting everything
#: after them. `berdl-upload.md` needs exactly this: it names two external scripts at line 729 and
#: then cites a script of its own, which resolves today, at 841. A declaration running to the end
#: of the file would stop checking that one, and a maintained citation that stops being checked is
#: the failure this whole check exists to prevent. Optional: a declaration with no end still runs
#: to the end of the file, which is what the historical section at the bottom wants.
EXTERNAL_DECLARATION_END = re.compile(r"<!--\s*/external-scripts\s*-->", re.IGNORECASE)
...
def unresolvable_scripts(paths: list[Path], root: Path) -> list[tuple[Path, int, str]]:
    ...
    from that declaration to the matching end marker, or to the end of the file if there is none,
    is skipped. The declaration is deliberately not file-wide: `berdl-upload.md` declares partway
    down, so treating it as file-wide exempted the maintained runbook above it, including a
    citation of a script this repository does own.
    """
    problems = []
